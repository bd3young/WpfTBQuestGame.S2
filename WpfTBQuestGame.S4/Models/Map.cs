...
namespace WpfTBQuestGame.S2.Models
{
    public class Map
    {
        ...
        public List<Npc> StandardNpcs
        {
            get { return _standardNpcs; }
            set { _standardNpcs = value; }
        }
        ...
        public string OpenLocationsByShip(int shipId)
        {
            string message = "The ship did nothing.";
            ...
            for (int row = 0; row < _maxRows; row++)
            {
                for (int column = 0; column < _maxColumns; column++)
                {
                    ...
                    if (mapLocation != null && mapLocation.RequiredShipId == shipId)
                    {
